...
namespace {
// https://stackoverflow.com/a/67625767
static uint16_t constexpr crc_table[] = {
...
std::uint16_t Crc16::operator()(std::uint8_t const* const buffer,
                                std::uint8_t const size) const noexcept {
    std::uint16_t crc = 0x00;
    if (buffer == nullptr || size == 0) return 0;
    for (std::size_t i = 0; i < size; i++) {
        crc = (crc << 8) ^ crc_table[((crc >> 8) ^ buffer[i]) & 0xff];
    }
    return crc;
}
}  // namespace utils
